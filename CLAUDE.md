...
- [x] **WU-24 four Agent Engine deployments** — *Prachit* (Vedant's ticket, done while he was
      asleep; `infra/agent_engine/`). `cn-ingest`, `cn-triage`, `cn-clearance`, `cn-enforcement`,
      each on its own service account. Resource names in `infra/agent_engine/deployed.json`, the
      answers they gave in `smoke_output.json`. **Redeploy with
      `python -m infra.agent_engine.deploy --runtime <name>`** — and read `DESIGN.md` §2.0 first,
      because three separate things about Agent Engine fail in ways that look like build problems
...
**Nothing is cut any more.** WU-24 was cut on 9 Sep and then un-cut the same day at Prachit's call:
the rules quote *"powered by Gemini and Google Cloud Agent Builder"*, and Cloud Run alone made that
a claim in prose. It is deployed. What remains uncut but unbuilt is P1/P2 sweep work — WU-26, WU-27,
WU-28, WU-32, WU-33 — and `consentinel/evidence/store.py`.

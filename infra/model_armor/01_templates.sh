...
echo
echo "NOTE: these flags are what gcloud 583 accepts. Enforcement takes only"
echo "enabled|disabled - there is no inspect-only at the template level, which"
echo "is why the flag/block asymmetry lives in consentinel/model_armor.py"
echo "instead. If a flag is rejected on a newer release, run"
echo "  gcloud model-armor templates create --help"
echo "and fix it here rather than in the Python - the module only needs the"
echo "templates to exist under these three names."
